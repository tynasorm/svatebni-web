...
export function Schedule() {
  return (
    <Section id="harmonogram" title="Harmonogram" subtitle="Průběh dne">
      <p className="mb-10 text-center font-body text-[0.95rem] font-normal leading-[1.85] text-warm-brown sm:mb-12 sm:text-base">
      ...
      <div className="relative mx-auto w-full max-w-lg">
        <div
          className="absolute top-2 bottom-2 left-[2.85rem] w-px bg-base-beige sm:left-[4.25rem] md:left-[5rem]"
          aria-hidden="true"
        />
        ...
        <ul className="space-y-8 sm:space-y-12">
          {schedule.map((item) => (
            ...
            >
              <time
                dateTime={item.time}
                className="w-12 shrink-0 pt-0.5 text-right font-body text-xs font-bold tabular-nums text-warm-brown sm:w-14 sm:text-sm md:w-16 md:text-base"
              >
                {item.time}
              </time>
              ...
              <p className="min-w-0 flex-1 pt-0.5 font-body text-[0.95rem] font-normal leading-[1.85] text-warm-brown sm:text-base">
                {item.event}
              </p>
            </li>
          ))}
